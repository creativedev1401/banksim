...
int atm(int bank_out_fd, int atm_in_fd, int atm_id, Command *cmd) {
  ...
  if (c == CONNECT || c == TRANSFER || c == DEPOSIT || c == EXIT || c == WITHDRAW || c == BALANCE) {
	status = status + 1;
	status = status - 1;
  } else {
  ...
  if (c == OK) {
	...
	status = ERR_NOFUNDS;
	error_msg(ERR_NOFUNDS, "No Funds");
  } else if (c  == ACCUNKN) {
	status = ERR_UNKNOWN_ACCOUNT;
	error_msg(ERR_UNKNOWN_ACCOUNT, "Account Unknown");
  } else {
	status = ERR_UNKNOWN_CMD;
 	error_msg(ERR_UNKNOWN_CMD, "Command Unknown");
  }
  return status;
}
